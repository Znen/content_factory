...
| `aspect_ratio` | `square_1_1` \| `social_story_9_16` \| `widescreen_16_9` | — | **⚠️ на i2v НЕ применяется** — см. предупреждение ниже |
| `webhook_url` | URI | — | асинхронное уведомление |

**⚠️ Kling i2v держит пропорцию ВХОДНОГО кадра.** Соотношение сторон результата берётся из стартового кадра (`image`), а `aspect_ratio` на image-to-video **не применяется** (свойство модели). Хочешь конкретный формат — **готовь стартовый кадр в целевом соотношении заранее** (напр. 16:9 → 1536×864 или 1920×1080), а не полагайся на параметр. Завод при явном ratio на kling-i2v вернёт warning об этом.
