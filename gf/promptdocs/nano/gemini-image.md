...
# ce-gemini-imagegen: dead link; johnlindquist@gemini-image: vision-скил — контент из официальных Google-доков
updated: 2026-07-06
---
# Как писать промпт для Nano Banana (Gemini image, `gemini-3-pro-image-preview`)
...
Aspect ratio и resolution — это НЕ часть текста промпта, это отдельные аргументы вызова (`aspect`, дефолт `9:16` задаёт python-обёртка gf — параметр функции в `gf/backends/nano.py`, не сам сервер). Держи их в голове при формулировке композиции (план кадра должен логически подходить под ориентацию), но не проговаривай числами внутри промпта.
...
| Референсы | аргумент `refs` (список путей до 4 файлов) | PNG/JPEG/WEBP; порядок в списке = порядок, в котором на них ссылается промпт |
